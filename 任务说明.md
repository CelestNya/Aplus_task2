...
- 2.2.3.3 转换LLaMA模型,并使用LLaMA.cpp运行
    ...
    首先部署好转换脚本（虽然网上有很多现成的gguf，但是为了可以更自由的选择模型，自己转换总是更灵活的） [转换脚本说明](libs/Aplus2-convertLLaMA/README.md)
    ...
    在量化后，模型或多或少会产生精度损失，一般的对话可以轻松胜任，但是一旦推理很容易陷入死循环，还是模型权重太少。从vlm对话中，两次对人物的思考都是我手动打断的，不能很好的推理出图中人物。不过在速度上，量化后的模型推理速度飞快，tps惊人的上百了。
